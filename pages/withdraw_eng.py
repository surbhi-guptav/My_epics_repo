...
if st.session_state.manual_input:
    with st.form("Withdraw_form"):
        ...
        Withdraw_amount = st.text_input("💵 Withdraw Amount", placeholder="Enter amount (₹)")
        
        st.subheader("📅 Date of Withdraw")
        Withdraw_date = st.date_input("Select Date", value=datetime.today())
        ...
        if Withdraw_amount and not Withdraw_amount.isdigit():
            errors.append("🔴 Withdraw amount must be a valid number.")

        # Submit button
        submit = st.form_submit_button("✅ Submit Withdraw")
        ...
        if submit:
            if errors:
                ...
                st.success("✅ Withdraw request submitted successfully!")
                st.write(f"Account Holder: {name}")
                st.write(f"Phone: {country_code} {phone_number}")
                st.write(f"Account Number: {account_number}")
                st.write(f"Amount: ₹{Withdraw_amount}")
                st.write(f"Date: {Withdraw_date}")

# *Voice Input Section*
elif st.session_state.voice_input:
    st.subheader("🎤 Voice Input")
    st.info("Example: 'My name is Rahul, my phone number is 9876543210, my account number is 987667543210I want to withdraw 500 rupees.'")
